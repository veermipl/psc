...
use App\Http\Requests\admin\data\caricom_cet\CreateCaricomCETHowItWorksRequest;
...
use App\Http\Requests\admin\data\caricom_cet\DeleteCaricomCETHowItWorksRequest;
use App\Http\Requests\admin\data\caricom_cet\DeleteCaricomCETObjectiveRequest;
use App\Http\Requests\admin\data\caricom_cet\UpdateCaricomCETHowItWorksRequest;
use App\Http\Requests\admin\data\caricom_cet\UpdateCaricomCETHowItWorksStatusRequest;
...
class CaricomCETController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $how_it_works = CaricomCET::orderBy('id', 'desc')->where('type', 'how_it_works')->get();
        ...
        $data['how_it_works_export_id'] = $how_it_works->pluck('id')->toArray();
    ...
    public function createHowItWorks()
    {
        ...
        return view('admin.data.caricom_cet.create_how_it_works');
    }

    public function saveHowItWorks(CreateCaricomCETHowItWorksRequest $request)
    {
        $this->authorize('data_create');
        ...
        return redirect()->route('admin.data.caricom-cet', ['tab' => $validated['type']])->with('success', 'How it works created successfully');
    }
    ...
    public function editHowItWorks($id)
    {
        ...
        $howItWorksData = CaricomCET::where('id', $id)->firstOrFail();

        $data['how_it_works'] = $howItWorksData;

        return view('admin.data.caricom_cet.edit_how_it_works', $data);
    }

    public function updateHowItWorks(UpdateCaricomCETHowItWorksRequest $request)
    {
        $this->authorize('data_edit');
        ...
        DB::transaction(function () use ($validated) {
            CaricomCET::where('id', $validated['how_it_works_id'])->update([
                'title' => $validated['title'],
                'content' => $validated['content'],
                'type' => $validated['type'],
                'status' => $validated['status'],
        ...
        return redirect()->route('admin.data.caricom-cet', ['tab' => $validated['type']])->with('success', 'How it works updated successfully');
    }

    public function deleteHowItWorks(DeleteCaricomCETHowItWorksRequest $request)
    {
        $this->authorize('data_delete');
        ...
        $data['error'] = false;
        $data['msg'] = 'How it works Deleted';

        return response()->json($data, 200);
    }

    public function updateHowItWorksStatus(UpdateCaricomCETHowItWorksStatusRequest $request)
    {
        $this->authorize('data_status_edit');
