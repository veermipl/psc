...
@section('title', 'COTED - List')
@section('header', 'COTED')

@section('content')

<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
    <div class="breadcrumb-title pe-3">COTED</div>
</div>
        ...
        <div class="card radius-10">
            <div class="card-body">
                <div class="p-4 border rounded">
                    <div id="">
                        <button class="btn btn-sm sub_page_link {{ $tab == 'main' ? 'btn-custom' : 'btn-outline-custom' }}"
                    ...
                    <div id="" class="pt-4">
                        <div class="collapse sub_page_body {{ $tab == 'main' ? 'show' : 'hide' }}" id="sub_page_body_main">
                            <form action="{{ route('admin.data.coted.update') }}" method="post" enctype="multipart/form-data" class="row g-3 needs-validation">
                                @csrf
                                @method('post')
                        ...
                        <div class="collapse sub_page_body {{ $tab == 'entrepreneurship_development' ? 'show' : 'hide' }}"
                            id="sub_page_body_entrepreneurship_development">
                            <div class="row">
                                <div class="col-lg-12 mb-3">
                                    <div class="d-flex justify-content-between">
                                        <a href="{{ route('admin.data.coted.create-entrepreneurship-development') }}" class="btn btn-primary btn-sm">
                                            <ion-icon name="add" role="img" class="md hydrated" aria-label="person add"></ion-icon>Create Entrepreneurship Development
                                        </a>

                                        @if ($entrepreneurship_development_export_id && count($entrepreneurship_development_export_id) > 0)
                                            <form action="" method="post" class="d-none">
                                                @csrf
                                                @method('post')

                                                <input type="hidden"
                                                    value="{{ implode(',', $entrepreneurship_development_export_id) }}"
                                                    name="export_id">
                                ...
                                <div class="table-responsive">
                                    <table id="cotedEntrepreneurshipDevelopmentTable"
                                        class="table table-sm table-borderless table-light" data-toggle="table" data-search="true"
                                        ...
                                        <tbody>
                                            @if ($entrepreneurship_development && count($entrepreneurship_development) > 0)
                                                @foreach ($entrepreneurship_development as $listKey => $listValue)
                                                        ...
                                                        <td>
                                                            <div class="tableOptions">
                                                                <span class="text-dark" title="Edit">
                                                                    <a
                                                                        href="{{ route('admin.data.coted.edit-entrepreneurship-development', $listValue->id) }}"><i
                                                                            class="fa fa-pencil"></i></a>
                                                                </span>
                                                                <span class="text-danger" title="Delete"
                                                                    lid="{{ $listValue->id }}" lrow="{{ $listKey }}"
                                                                    id="deleteListBtn">
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '#listStatus', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.data.coted.update-entrepreneurship-development-status') }}",
                            method: 'POST',
                            data: {
                                _method: 'post',
                                _token: '{{ csrf_token() }}',
            ...
            $(document).on('click', '#deleteListBtn', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.data.coted.delete-entrepreneurship-development') }}",
                            method: 'POST',
                            data: {
                                _method: 'post',
                                _token: '{{ csrf_token() }}',
