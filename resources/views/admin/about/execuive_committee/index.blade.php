...
@section('title', 'Executive Committee - List')
@section('header', 'Executive Committee')

@section('content')

    <div class="page-breadcrumb d-sm-flex align-items-center mb-3">
        <div class="breadcrumb-title pe-3">Executive Committee</div>
    </div>
            ...
            <div class="card radius-10">
                <div class="card-body">
                    <div class="p-4 border rounded">
                        <form action="{{ route('admin.about-us.executive-committee.filter') }}" method="post"
    ...
    <div class="row">
        <div class="col-lg-12 mb-3">
            <div class="d-flex justify-content-between">
                <a href="{{ route('admin.about-us.executive-committee.create') }}" class="btn btn-primary btn-sm">
                    <ion-icon name="person-add-outline" role="img" class="md hydrated" aria-label="person add"></ion-icon>
                    Create Executive Committee
                </a>

                @if ($export_id && count($export_id) > 0)
                    <form action="{{ route('admin.about-us.executive-committee.export') }}" method="post" class="d-none-">
                        @csrf
                        @method('post')
                                ...
                                @if ($list && count($list) > 0)
                                    @foreach ($list as $listKey => $listValue)
                                        <tr class="tr_row_{{ $listKey }}">
                                            ...
                                            <td>
                                                <img src="{{ $listValue->image && isset($listValue->image) ? asset('storage/' . $listValue->image) : asset('storage/default/no_image.jpg') }}" class="tableImg" alt="">
                                            </td>

                                            <td>
                                                <a href="{{ route('admin.about-us.executive-committee.show', $listValue->id) }}"
                                            ...
                                            <td>
                                                <div class="tableOptions">
                                                    <span class="text-dark" title="Edit">
                                                        <a
                                                            href="{{ route('admin.about-us.executive-committee.edit', $listValue->id) }}"><i
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            $(document).on('click', '#listStatus', function(e) {
                ...
                Swal.fire({
                    ...
                    if (result.isConfirmed) {
                        $.ajax({
                            url: "{{ route('admin.about-us.executive-committee.status') }}",
                            method: 'POST',
                            data: {
                                _method: 'post',
                                _token: '{{ csrf_token() }}',
            ...
            $(document).on('click', '#deleteListBtn', function(e) {
                ...
                var lid = $(this).attr('lid');
                var lrow = $(this).attr('lrow');
                var url = `{{ url('/admin/about-us/executive-committee/${lid}') }}`;
