...
@section('header', 'Update News')

@section('content')

    <div class="p-3 bg-white">

        <h5 class="fw-bold">Update News</h5>

        <div class="pt-5">
            <form action="{{ route('admin.media-center.news.update', $news->id) }}" method="post" enctype="multipart/form-data">
                @csrf
                @method('patch')
                ...
                <div class="d-flex">
                    <div class="form-group col-md-6">
                        ...
                        <input type="text" id="name" class="form-control" name="title" placeholder="Enter title"
                            value="{{ old('title', $news->title) }}" maxlength="50">
                                ...
                                @foreach ($news_files as $fileKey => $fileValue)
                                    @if ($fileValue)
                                        ...
                                        <div class="editFileWrapper" file_row_url="{{ $fileValue }}">
                                            ...
                                            @endif
                                            <button class="btn btn-sm btn-outline-danger mt-2 deleteFileBtn" type="button" id="{{ $news->id }}" file_url="{{ $fileValue }}">
                                                Delete
                                            </button>
                                        </div>
                                    @endif
                ...
                <div class="form-group col-md-12 text-right">
                    <button class="btn btn-sm btn-custom" type="submit">Update News</button>
                </div>
            </form>
        </div>
    </div>
        ...
        $(document).on('click', '.deleteFileBtn', function(e) {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: "{{ route('admin.media-center.news.delete-file') }}",
                        method: 'POST',
                        data: {
                            _method: 'post',
                            _token: '{{ csrf_token() }}',
